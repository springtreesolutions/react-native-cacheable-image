...
import { Image, NetInfo, Platform, View } from 'react-native';
import RNFS, { DocumentDirectoryPath } from 'react-native-fs';

// add "react-native-responsive-image": "^2.0.0", to package.json before import and nmp i
...
export default class CacheableImage extends React.Component {
  ...
  _processSource = (source, skipSourceCheck) => {
    if (
      this.props.storagePermissionGranted &&
      source !== null &&
      source != '' &&
      ...
      const cacheKey =
        SHA1(cacheable) + (type.length < url.pathname.length ? '.' + type : '');
  ...
  render() {
    if (
      (!this.state.isRemote && !this.props.defaultSource) ||
      !this.props.storagePermissionGranted
    ) {
    ...
    // custom place holder here
    return (
      <View {...props} {...activityIndicatorProps} style={style}>
  ...
  renderCache() {
    ...
    // Was responsive image before fork !!
    return (
      <Image
        {...props}
